 ...
 * 动态频率策略：
 * - 默认闲置：60 秒
 * - 有正在执行的任务（Yuanzi status == executing）：10 秒
 * - 屏幕亮起时：立即触发一次并临时加速到 10 秒
 */
class YuanziPollReceiver : BroadcastReceiver() {

    companion object {
        const val ACTION_POLL = "com.nous.widgetmcp.action.YUANZI_POLL"
        const val EXTRA_SCREEN_ON = "screen_on"

        fun createIntent(context: Context, screenOn: Boolean = false): Intent {
            return Intent(context, YuanziPollReceiver::class.java).apply {
                action = ACTION_POLL
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (action == Intent.ACTION_BOOT_COMPLETED) {
            AppLogger.i("YUANZI_POLL", "boot completed")
            if (YuanziConfig.enabled) YuanziPollScheduler.scheduleNext(context)
            return
        }
        ...
        AppLogger.i("YUANZI_POLL", "trigger screenOn=$screenOn")
        ...
        // 亮屏时临时安排短间隔的下一轮
        if (screenOn) {
            YuanziPollScheduler.schedule(context, YuanziPollScheduler.FAST_INTERVAL_MS)
        }
    }
}

object YuanziPollScheduler {
    ...
    fun schedule(context: Context, intervalMs: Long) {
        ...
        val intent = YuanziPollReceiver.createIntent(context)
        ...
        when {
            Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && canExact -> {
                ...
                AppLogger.i("YUANZI_POLL", "scheduled exact in ${intervalMs}ms")
            }
            Build.VERSION.SDK_INT >= Build.VERSION_CODES.M -> {
                ...
                AppLogger.i("YUANZI_POLL", "scheduled inexact in ${intervalMs}ms")
            }
            else -> {
                am.set(AlarmManager.ELAPSED_REALTIME_WAKEUP, triggerAt, pending)
                AppLogger.i("YUANZI_POLL", "scheduled legacy in ${intervalMs}ms")
            }
        }
    }
    ...
    private fun resolveInterval(context: Context): Long {
        // 未来可读取 Yuanzi status 或本地任务状态来决定；目前默认 60s
