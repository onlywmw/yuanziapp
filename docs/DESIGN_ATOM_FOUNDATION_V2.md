...
健康指标实时采集，按固定周期（每日）计算评分:
  ...
  < 50 分     连续 3 次评分 < 50 → `offline`    防网络波动误判
...
系统处理:
  ...
  · `file_ref` 指向临时存储 (工作流结束后 TTL 24h, 到期自动清理; 工作流异常退出 TTL 仍然生效)
...
测量条件 (统一口径):
  · 单次执行 (非并发)
  · 参考设备: Pixel 6 (Android 14), 其他设备用相对值
  · 非首次 (预热后)
...
注册或使用原子 A 时, 系统分析其依赖的健康状况:
  ...
  权重 = 1 / (1 + 依赖深度)  ← 深度越深权重越低
  (作者可在 meta 中声明某依赖为关键依赖, 权重 ×2)

  A 依赖:
    B (running, 健康度 95)  ✅  权重 1.0, 贡献 95
    C (degraded, 健康度 60) ⚠️  权重 1.0, 贡献 60
    D (deprecated)          ❌  权重 2.0 (关键依赖), 贡献 0
...
P3 (远期):
  灰度发布 (`canary`) — 需要服务网格/网关级流量控制, 端侧系统暂不实施
```
