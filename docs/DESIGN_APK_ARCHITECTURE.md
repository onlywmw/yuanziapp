...
│  ┌─────────────┐     HTTP       ┌──────────────┐ │
│  │   APK        │ ←──────────→  │ Python 后端   │ │
│  │  (Kotlin)   │  localhost    │ (Termux)      │ │
│  │             │   :8081 REST  │              │ │
│  │  知识图谱    │   :8080 MCP   │  api.py      │ │
│  │  原子市场    │               │  registry.py │ │
│  │  工作流画布  │               │  agent.db    │ │
│  └─────────────┘               └──────────────┘ │
│                                                   │
...
YuanziApi.fetchGraph()
  → GET http://127.0.0.1:8081/atoms?status=registered
...
widgetmcp_src/java/com/nous/widgetmcp/
  ...
  核心层:
    ...
    McpServer.kt             MCP JSON-RPC 服务器 :8080
  ...
  Yuanzi 桥接:
    yuanzi/YuanziApi.kt       REST API 调用
    yuanzi/YuanziConfig.kt    连接配置
