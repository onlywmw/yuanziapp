...
# -*- coding: utf-8 -*-
"""
把 awslabs/mcp-main 仓库里的 MCP 服务器批量导入 Hermes 图谱。

策略：
- 每个 src/*-mcp-server/ 目录视为一个能力原子
- 从 README.md 和 pyproject.toml 提取名称、描述、工具列表
- 直接写入 Hermes core 的 SQLite 数据库 atoms 表
...
# Hermes DB 路径（在 Termux/Debian 中）
...
def import_to_hermes(atoms: List[Dict[str, Any]]):
    """写入 Hermes SQLite 数据库"""
...
def main():
    ...
    print(f"\n=== 导入 Hermes DB: {DB_PATH} ===")
    import_to_hermes(atoms)
    print("导入完成")
