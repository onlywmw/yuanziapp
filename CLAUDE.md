...
Yuanzi 不是一个普通的工具合集，而是一个**原子化服务生态系统的操作系统级实验**：

1. **万物皆原子**：每一个能力（API 调用、数据处理、AI 模型、浏览器控制、数据库查询）都被建模为独立、可组合的"原子"（Atom）
2. **知识图谱即界面**：原子之间的关系不是隐藏的后端逻辑，而是在 Android 设备上可视化的知识图谱
3. **Android 优先**：目标运行环境是 Android + Termux，不是传统服务器或 Web 应用
4. **去中心化注册**：原子通过内容指纹（content_hash）和身份指纹（identity_hash）去重，不依赖中心化命名服务
