...
try:
    ...
    print("result:", {k: r.get(k) for k in ("success", "probe_status", "new_status", "ok")})
...
print("offline probe-fail ->", r["new_status"],
      "| note: offline->unreachable not in set_atom_status.allowed_transitions")

print()
print("== 6. rejected atom probe (only-记录 claim) ==")
c6 = fresh()
registry.submit_atom(c6, {**mk("com.qa.rej"), "runtime": {"health_url": "http://127.0.0.1:1/x"}})
...
print("rejected probe ->", r["new_status"], "| runtime recorded:",
      atom["runtime"].get("last_probe_status"), "| audit:",
