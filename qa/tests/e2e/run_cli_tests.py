...
rec(
    "TC-CLI-002",
    r.exit_code == 0 and created.exists() and not missing,
...
rec(
    "TC-CLI-003",
    r2.exit_code == 0 and "com.qa.demo@0.1.0" in r2.output,
...
graceful = (
    r2.exit_code != 0 and "traceback" not in (r2.output + (r2.stderr or "")).lower()
)
exc = getattr(r2, "exception", None)
rec(
    "TC-CLI-005",
    r2.exit_code != 0 and exc is None,
...
rec(
    "TC-CLI-006",
    r.exit_code == 0 and "com.example.sum@0.1.0" in r.output,
    r.output.strip(),
)
...
rec(
    "TC-CLI-007",
    r.exit_code == 1 and "meta.yaml not found" in (r.output + r.stderr),
    f"exit={r.exit_code}",
)
...
rec(
    "TC-CLI-008",
    r.exit_code == 1 and "missing required files" in out and "test_health.py" in out,
    f"exit={r.exit_code} out={out.strip()}",
)
...
rec(
    "TC-CLI-009",
    r.exit_code == 1 and "validation failed" in (r.output + r.stderr),
    f"exit={r.exit_code}",
)
...
(md / "meta.yaml").write_text(
    yaml.safe_dump(base_meta(kernel_type="markdown_rules")), encoding="utf-8"
)
r = runner.invoke(app, ["validate", str(md)])
rec(
    "TC-CLI-010",
    r.exit_code == 1 and "rules.md" in (r.output + r.stderr),
...
rec(
    "TC-CLI-012",
    r.exit_code == 0 and "8 passed" in r.output,
...
rec(
    "TC-CLI-013",
    r.exit_code == 0 and "fast mode" in r.output and "5 passed" in r.output,
    f"exit={r.exit_code} tail={r.output.strip()[-120:]}",
)
...
rec(
    "TC-CLI-014",
    r.exit_code == 0
    and "is a valid Yuanzi atom" not in r.output
    and "8 passed" in r.output,
    f"exit={r.exit_code}",
)
...
core.write_text(
    core.read_text(encoding="utf-8").replace("return a + b", "return a + b + 1"),
    encoding="utf-8",
)
...
rec(
    "TC-CLI-017",
    p0 == "rejected" and p65536 == "rejected" and pok == "accepted",
...
r = runner.invoke(
    ...
    [
        ...
        "com.qa.custom",
        "-t",
        str(REPO / "yuanzi-atom-templates"),
        "-o",
        str(tmp),
    ],
)
rec(
    "TC-CLI-018",
    r.exit_code == 0 and (tmp / "com.qa.custom" / "meta.yaml").exists(),
    f"exit={r.exit_code}",
)
...
rec(
    "TC-CLI-019",
    "Usage" in r.output or "usage" in r.output.lower(),
    f"exit={r.exit_code}",
)
