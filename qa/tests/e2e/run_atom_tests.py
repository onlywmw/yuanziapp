...
rec(
    "TC-ATM-002",
    r.get("status") == "success" and r["data"]["result"] == 0.0,
    json.dumps(r),
)
...
rec(
    "TC-ATM-004",
    True,
    f"documented: 1e308+1e308 -> {json.dumps(r)} (inf returned as success={is_inf})",
)
...
rec(
    "TC-ATM-005",
    r == {"status": "success", "data": {"parts": ["a", "b", "c"], "count": 3}},
    json.dumps(r),
)
...
rec(
    "TC-ATM-006",
    r.get("data", {}).get("parts") == ["a", "b", "c,d"] and r["data"]["count"] == 3,
    json.dumps(r),
)
...
rec(
    "TC-ATM-009",
    r.get("status") == "error" and "file not found" in r.get("message", ""),
    json.dumps(r),
)
...
rec(
    "TC-ATM-010",
    r.get("status") == "error" and "file too large" in r.get("message", ""),
    json.dumps(r),
)
...
rec(
    "TC-ATM-012",
    r.get("status") == "error"
    and "missing required field: path" in r.get("message", ""),
    json.dumps(r),
)
...
rec(
    "TC-ATM-013",
    r.get("status") == "success"
    and r["data"]["status_code"] == 200
    and "yuanzi" in r["data"]["text"],
    json.dumps(r)[:200],
)
...
rec(
    "TC-ATM-014",
    r.get("status") == "error"
    and "missing required field: url" in r.get("message", ""),
    json.dumps(r),
)
...
rec(
    "TC-ATM-015",
    r.get("status") == "success" and len(r["data"]["text"]) == 100,
...
rec("TC-ATM-018", all(conform.values()), f"/meta ids={ids}")
