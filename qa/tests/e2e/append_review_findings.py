...
REVIEW_ROWS = [
    (
        ...
        DATE,
        WHO,
        "com.qa.beta 与 alpha 同能力不同 id 仍 success；content_hash 未持久化（无列、行内无），去重不可执行也不可查询",
        "",
    ),
    (
        "TC-REV-002",
        ...
        DATE,
        WHO,
        "TypeError: '<=' not supported between 'int' and 'NoneType'；批量探测会被单行数据整体搞死",
        "",
    ),
    (
        "TC-REV-003",
        ...
        DATE,
        WHO,
        "http_503 -> unreachable，未崩溃（环境代理相关，行为可接受）",
        "",
    ),
    (
        "TC-REV-004",
        ...
        DATE,
        WHO,
        "no_endpoint 分支无 probe 审计记录，与'每次探测记审计日志'声明不符",
        "",
    ),
    (
        "TC-REV-005",
        ...
        DATE,
        WHO,
        "offline->unreachable 实际发生但不在 set_atom_status 流转表；probe 绕过状态机校验，双路径并存",
        "",
    ),
    (
        "TC-REV-006",
        ...
        DATE,
        WHO,
        "rejected 保持，runtime 记录 connection_error，probe 审计已写（符合设计）",
        "",
    ),
    (
        "TC-REV-007",
        "Review",
        "P1",
        "install-hooks 无界向上搜索副作用",
        "5",
        "home 存在配置+home 是 git 仓库",
        "Completed",
        "❌ FAILED",
        "BUG-015",
        DATE,
        WHO,
        "test_install_hooks_no_config 本机失败（exit 0≠1）；真实 pip install + 钩子植入 home 仓库；环境依赖测试",
        "",
    ),
    (
        "TC-REV-008",
        ...
        DATE,
        WHO,
        "bridge 31/31 通过；CLI 8/9 通过（test_install_hooks_no_config 环境依赖失败）；'累计40 passed' 在本机不成立",
        "",
    ),
    (
        "TC-REV-009",
        ...
        DATE,
        WHO,
        "probing 仅出现在流转表/_PROBEABLE_STATUSES，无任何代码路径设置该状态 → 死状态",
        "",
    ),
]
...
NEW_BUGS = [
    (
        "BUG-014",
        "probe_atom 遇 file:// 等非 HTTP scheme 崩溃（TypeError），批量探测整体中断",
        "P1",
        "mcp-yuanzi-bridge / registry.probe_atom",
        "TC-REV-002",
        "Open",
        DATE,
        WHO,
        ...
        ENV,
        "",
        "urlopen 前校验 scheme；批量循环内捕获单原子异常",
        "",
        "",
        "",
    ),
    (
        "BUG-015",
        "install-hooks 向上搜索无边界 + 负路径测试未 stub，产生真实本机副作用",
        "P1",
        "yuanzi-cli / install-hooks + 测试",
        "TC-REV-007",
        "Open",
        DATE,
        WHO,
        ...
        ENV,
        "",
        "搜索遇 .git 即停（或不越过 repo 边界）；测试补 stub；建议增加 --dry-run",
        "",
        "",
        "",
    ),
    (
        "BUG-016",
        "content_hash/identity_hash 不持久化，能力去重仍不可执行（BUG-006 未闭环）",
        "P1",
        "mcp-yuanzi-bridge / registry",
        "TC-REV-001",
        "Open",
        DATE,
        WHO,
        ...
        ENV,
        "",
        "signature_hash 改用 content_hash 作主去重键，或新增 content_hash 列+唯一/警告逻辑；明确两提交的最终语义",
        "",
        "",
        "",
    ),
    ...
    (
        "BUG-018",
        "probe 的 not_found/no_endpoint 分支不写审计，与提交说明不符",
        "P2",
        "mcp-yuanzi-bridge / registry.probe_atom",
        "TC-REV-004",
        "Open",
    ...
    (
        ...
        ENV,
        "",
        "probe 内部调用 set_atom_status；同步更新流转表与注释",
        "",
        "",
        "",
    ),
    (
        "BUG-020",
        "probe 对注册表中的任意 URL 发起请求，无 scheme/地址校验（信任边界）",
        "P2",
        "mcp-yuanzi-bridge / registry.probe_atom",
        "TC-SEC-002",
        "Open",
    ...
    (
        "BUG-021",
        "probe_atoms CLI 永远 exit 0、串行执行、--json 无汇总，不可用于监控",
        "P2",
        "mcp-yuanzi-bridge / probe_atoms.py",
        "TC-REV-008",
        "Open",
        DATE,
        WHO,
