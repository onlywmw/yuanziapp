...
rec("TC-CLI-012", r.returncode == 0 and "8 passed" in out
    and "is a valid Yuanzi atom" in out,
    f"rc={r.returncode} valid_line={'is a valid Yuanzi atom' in out} 8passed={'8 passed' in out}")

# TC-CLI-013 --fast
r = run([str(YUANZI), "test", "--fast", str(EXAMPLE)])
out = r.stdout + r.stderr
rec("TC-CLI-013", r.returncode == 0 and "fast mode" in out and "5 passed" in out,
    f"rc={r.returncode} 5passed={'5 passed' in out}")

# TC-CLI-014 --no-validate
r = run([str(YUANZI), "test", "--no-validate", str(EXAMPLE)])
out = r.stdout + r.stderr
rec("TC-CLI-014", r.returncode == 0 and "8 passed" in out
    and "is a valid Yuanzi atom" not in out, f"rc={r.returncode}")
...
core.write_text(src.replace("return {\"result\": a + b}",
                            "return {\"result\": a + b + 1}"), encoding="utf-8")
r = run([str(YUANZI), "test", "--no-validate", str(failatom)])
out = r.stdout + r.stderr
rec("TC-CLI-015", r.returncode != 0 and "failed" in out,
    f"rc={r.returncode} failed_in_out={'failed' in out}")

# TC-CLI-020 pytest from repo root (CWD robustness of relative-path tests)
r = run([str(PYTEST), "-m", "pytest", "yuanzi-cli/tests", "-v", "--no-header"], cwd=REPO)
out = r.stdout + r.stderr
rec("TC-CLI-020", r.returncode == 0,
    f"rc={r.returncode} tail={out.strip()[-200:]}")
