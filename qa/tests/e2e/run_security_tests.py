...
leaked = r1.get("status") == "success" and "localhost" in r1.get("data", {}).get(
    "content", ""
)
rec(
    "TC-SEC-001",
...
ssrf_http = (
    r_http.get("status") == "success" and "INTERNAL-SECRET" in r_http["data"]["text"]
)
...
for server in sorted(ATOMS.glob("*/server.py")):
    ...
    auth[server.parent.name] = bool(
        re.search(r"token|api_key|Authorization|auth", src, re.I)
    )
tpl = (
...
rec(
    "TC-SEC-003",
    not (all_wild and not any(auth.values())),
...
rec(
    "TC-SEC-005",
    len(r1) == 0 and len(r2) == 0 and still == 1 and err is None,
...
uses = []
for p in [
    REPO / "yuanzi-cli" / "yuanzi_cli" / "meta.py",
    REPO / "yuanzi-atom-templates" / "{{cookiecutter.atom_id}}" / "server.py",
...
rec(
    "TC-SEC-007",
    len(hits) == 0 and not personal_path,
...
atom_leak = '"message": str(e)' in (ATOMS / "atom-math-sum" / "core.py").read_text(
    encoding="utf-8"
)
rec(
    "TC-SEC-008",
    not (tpl_leak or atom_leak),
...
lock_exists = any(
    (REPO / n).exists()
    for n in ["requirements.lock", "poetry.lock", "uv.lock", "requirements.txt"]
)
rec(
    "TC-SEC-009",
    lock_exists,
