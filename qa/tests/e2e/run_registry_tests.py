...
def mk(atom_id, name="Atom", fn=("f1",), desc="d"):
    return {
        ...
        "architecture": {"type": "skill", "runtime": "python", "interface": "std-atom-http-v1",
                         "dependencies": []},
        "ownership": {"team": "qa"},
    }
...
rec("TC-REG-001", r.get("success") and r["status"] == "submitted" and len(r["signature"]) == 32,
    str(r))

# TC-REG-002 duplicate signature, different atom_id
r2 = registry.submit_atom(conn, mk("com.qa.beta"), actor="qa")  # same functions/type/runtime
rec("TC-REG-002", not r2.get("success") and r2.get("error") == "duplicate_signature", str(r2))
...
rec("TC-REG-003", r3.get("success") and atom["description"] == "updated"
    and sum(1 for l in logs if l["action"] == "submit") == 2,
    f"desc={atom['description']} submits={sum(1 for l in logs if l['action']=='submit')}")

# TC-REG-004 approve -> registered
r = registry.review_atom(conn, "com.qa.alpha", True, reviewer="qa", score=9)
atom = registry.get_atom(conn, "com.qa.alpha")
rec("TC-REG-004", r.get("success") and r["status"] == "registered"
    and atom["review_score"] == 9, str(r))
...
rec("TC-REG-008", not r.get("success") and r.get("error") == "invalid_transition", str(r))

# TC-REG-009 list_atoms filter by status
rows = registry.list_atoms(conn, status="running")
rec("TC-REG-009", len(rows) == 1 and rows[0]["atom_id"] == "com.qa.alpha",
    f"running={len(rows)}")

# TC-REG-010 list_atoms search
rows = registry.list_atoms(conn, search="gamma")
rec("TC-REG-010", len(rows) == 1 and rows[0]["atom_id"] == "com.qa.gamma",
...
rec("TC-REG-012", len(rows) == 0 and total >= 3,
...
rec("TC-REG-014", r.get("success") and st == "submitted",
