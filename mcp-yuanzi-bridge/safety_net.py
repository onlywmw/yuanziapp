...
文档要求、但引擎侧尚未提供的接缝（以注入解耦）：
- abort_node：卡死恢复时调用 engine.abort_node(node_id)（第四节卡死修复）。
  get_safety_net() 单例经 _engine_abort_node 惰性对接：engine.py 未提供
  abort_node（或不可导入）时等价于未注入回调的 no-op，安全网行为不变。
- 权重/集群配置的实际应用：安全网只产出动作（Action）与快照数据，由引擎执行。
- 通知送达：文档多处要求"通知用户"，通知文案由本模块生成并经 CheckReport/
  events() 输出；api.py 端点由接线任务挂接（走 get_safety_net() 单例）。
...
_safety_net: Optional[SafetyNet] = None
_safety_net_lock = threading.Lock()


def _engine_abort_node(node_id: str) -> bool:
    """卡死恢复的 abort 回调（惰性 import engine + try/except 兜底）。

    engine.py 未提供 abort_node（或模块不可导入）时返回 False——等价于
    未注入回调的 no-op：节点仍被移出在飞登记、记入 aborted_nodes、日志
    记录，安全网自身行为不变（第四节卡死修复）。
    """
    try:
        import engine  # 惰性导入：engine 可能不存在或与安全网并行落地
    except Exception:  # noqa: BLE001
        return False
    abort = getattr(engine, "abort_node", None)
    if not callable(abort):
        return False
    try:
        return bool(abort(node_id))
    except Exception:  # noqa: BLE001 - abort 失败不拖垮安全网
        logger.exception("engine.abort_node raised for %s", node_id)
        return False


def get_safety_net() -> SafetyNet:
    """进程级安全网单例；abort_node 回调惰性对接 engine.abort_node。"""
    global _safety_net
    if _safety_net is None:
        with _safety_net_lock:
            if _safety_net is None:
                _safety_net = SafetyNet(abort_node=_engine_abort_node)
    return _safety_net


def reset_safety_net() -> None:
    """重置单例（测试用；不影响已持有旧实例引用的调用方）。"""
    global _safety_net
    with _safety_net_lock:
        _safety_net = None
