...
CORE_PORT = int(os.environ.get("HERMES_CORE_PORT", "8080"))
CORE_HOST = os.environ.get("HERMES_CORE_HOST", "127.0.0.1")
...
def route_health() -> Tuple[int, Dict[str, Any]]:
    return 200, ok_response({"status": "ok", "version": "v1", "atom": "hermes-core"})
...
def route_graph() -> Tuple[int, Dict[str, Any]]:
    ...
    nodes = [
        {"id": "hermes-core", "label": "Hermes Core", "type": "core", "status": "online"},
        {"id": "widgetmcp", "label": "组件 MCP", "type": "client", "status": "online"},
    ]
    edges = [
        {"source": "widgetmcp", "target": "hermes-core"},
    ]
    ...
    for cap in caps:
        ...
        for atom in atoms:
            if atom["atom_id"] == target or atom["label"] == target:
                atom_id = atom["atom_id"]
                break
        if atom_id and atom_id != "hermes-core":
            edges.append({"source": "hermes-core", "target": atom_id, "label": cap["tool_id"]})
...
class CoreHandler(BaseHTTPRequestHandler):
    def log_message(self, format: str, *args: Any) -> None:
        print(f"[Hermes-CORE] {self.address_string()} - {format % args}")
...
def main() -> None:
    ...
    print(f"[Hermes-CORE] listening on {CORE_HOST}:{CORE_PORT}")
    try:
